...
<body>
    <div class="container">
        ...
        <div class="header">
            <h1>🧺 Londri</h1>
            <p class="subtitle">Tempat Cuci-Mencuci dengan Harga Ekonomis</p>
        </div>
        ...
        <div class="main-content">
            ...
            <div class="card">
                ...
                <form method="POST" action="{{ route('trans.store') }}" id="transactionForm" class="needs-validation" novalidate>
                    @csrf
            ...
            <div class="card">
                ...
                <div class="transaction-list" id="transactionHistory">
                    ...
                    <div class="transaction-item">
                        <h4>{{ $order->code }} - {{ $order->name }}</h4>
                        <p>📞 {{ $order->phone }}</p>
                        <p>🛍️
                            @foreach ($orders as $details => $detail)
                            {{ $detail->service_name }} - {{ $detail->qty }} kg,
                            @endforeach
                        </p>
                        <p>💰 Rp {{ $order->total }}</p>
                        <p>📅 {{ $order->created_at }}</p>
                        <span class="status-badge status-{{ $customer->customer_status }}">{{ $customer->status_text }}</span>
                    </div>
                    @endforeach
                </div>
    ...
    <script>
        ...
        function processTransaction() {
            ...
            const transaction = {
                id_customer = optionCostumer.value,
            ...
            const token = dosument.querySelector('meta[name="csrf-token"]').getAttribute("content");
            fetch("/trans", {
                method:"POST",
                headers:{
